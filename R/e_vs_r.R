...
#' standard deviation, and highest density interval for each bootstrap method.
...
e_vs_r <- function(data, n_bootstraps = 10000, check_size = TRUE,
  ...
  exact_result <- exact_bootstrap(data, n_bootstraps, check_size, anon, lb = lb,
                                  ub = ub, density_args)
  reg_result <- reg_bootstrap(data, n_bootstraps, anon, lb = lb, ub = ub, density_args)

  # Create summary table
  summary_table <- rbind(as.data.frame(exact_result$stats),
                         as.data.frame(reg_result$stats))
  summary_table$Method <- c("exact_bootstrap", "reg_bootstrap")
